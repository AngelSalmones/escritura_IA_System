...
if not path.isfile('siquiero.model'):
	...
	# model.add(tf.keras.layers.Dense(528, activation=tf.nn.relu))
	# model.add(tf.keras.layers.Dense(528, activation=tf.nn.relu))
	# model.add(tf.keras.layers.Dense(528, activation=tf.nn.relu))
	...
	model.fit(images_training, labels_training, epochs=5)
	
	model.save('Entrenado1prueba.model')

else:
	model = tf.keras.models.load_model('siquiero.model')

val_loss, val_acc = model.evaluate(images_test, labels_test)
predictions = model.predict('3.jpeg')
print(np.argmax(predictions[0]))
plt.imshow('3.jpeg', cmap='binary')
plt.show()
